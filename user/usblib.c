...
void USBLIB_Init(void) {
    ...
    GPIOA->ODR |= GPIO_ODR_ODR12; // DP UP
}
...
const uint8_t USB_DEVICE_DESC[] = {
        ...
        (uint8_t)0x00,                      //    bcdUSB 2.0
        (uint8_t)0x02,                      //    bcdUSB 2.0
...
_USB_STRING_(wsVendor, L"malikalhack")
_USB_STRING_(wsProd, L"CDC_Device")
_USB_STRING_(wsSN, L"2022-0012-10")
...
void USBLIB_GetDescriptor(USBLIB_SetupPacket *SPacket) {
    ...
    case USB_CFG_DESC_TYPE: {
        uint16_t size;
        size = sizeof(USBD_CDC_CFG_DESCRIPTOR);
        if( size > SPacket->wLength) {
            size = SPacket->wLength;
        }
        USBLIB_SendData(0, (uint16_t *)&USBD_CDC_CFG_DESCRIPTOR, size);
        break;
    }
    ...
    case USB_STR_DESC_TYPE: {
        ...
        uint8_t i;
        for (i = 0; i < SetupPacket->wValue.L; i++) {
